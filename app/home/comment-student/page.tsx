...
const CommentStudent = () => {
  ...
  const columns: ColumnDef<any, any>[] = [
    ...
    columnHelper.accessor('score', {
      header: () => <span>Điểm học sinh</span>,
      cell: (info) => (
  ...
  const getData = async () => {
    try {
      const query = {
        ...
        id: choseFilter.key || '',
        type: choseFilter.type || 'student',
        pageSize: pageSize,
      };
      const res =
        await api.functional.comment.query.student.getStudentCommentWithQuery(
          connection,
          query,
        );
      if (res.data) {
  ...
  useQuery({
    queryKey: ['comment-student', pageNumber, choseFilter, pageSize],
    queryFn: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      api.functional.comment.student.removeTeacher(connection, id),
    onSuccess: () => {
  ...
  }, [displayColumns, listStudent, listTeacher, listClass, choseFilter]);

  return (
    <div>
      <div className="flex justify-between">
        <div className="items-center leading-10 text-[20px] font-semibold">
          Danh sách đánh giá học sinh
        </div>
