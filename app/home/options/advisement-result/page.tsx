...
const StudyGoal = () => {
  ...
  const getData = async () => {
    ...
    try {
      const res = await api.functional.advisement_result.findAll(
        connection,
        query,
      );
      if (res.data) {
      ...
      console.log(err);
      toast.error('Tải danh sách lưa chọn thất bại');
    }
    return 1;
  };
      ...
      <Popup open={openAdd} closeOnDocumentClick={false}>
        ...
        <AddOption
          ...
          addFunction={(data: any) =>
            api.functional.advisement_result.create(connection, data)
          }
        />
      </Popup>
      <Popup open={openUpdate} closeOnDocumentClick={false}>
        ...
        <AddOption
          updateFunction={(data: any) =>
            api.functional.advisement_result.update(
              connection,
              selected.id,
              data,
            )
