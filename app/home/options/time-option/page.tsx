...
import AddTimeOption from '@/components/AddTimeOption';
...
const StudyGoal = () => {
  ...
  const getData = async () => {
    ...
    try {
      const res = await api.functional.time_option.findAll(connection, query);
      if (res.data) {
  ...
  return (
    <>
      <Popup open={openAdd} closeOnDocumentClick={false}>
        ...
        <AddTimeOption closeModal={closeModal} getData={getData} />
      </Popup>
